...
class SpaceInvaders:
    ...
    def draw_border(self):
        ...
        # Cabinet frame (around the screen)
        screen_frame_offset = 2
        cabinet_depth = 3 # Define depth once

        # Calculate key X coordinates
        cabinet_front_left_x = self.start_x - screen_frame_offset
        cabinet_front_right_x = self.start_x + self.game_width + screen_frame_offset
        right_depth_x = cabinet_front_right_x + cabinet_depth
        
        # top cabinet border to back edge
        for x in range(cabinet_front_left_x - 1, cabinet_front_right_x + 1):
            self.add_char_safe(self.start_y - screen_frame_offset - 1, x, '^')
        self.add_char_safe(self.start_y - screen_frame_offset - 1, cabinet_front_right_x + 2, '\\')
        self.add_string_safe(self.start_y - screen_frame_offset, cabinet_front_left_x + 5, 'Space Invaders 👾')
        self.add_string_safe(self.start_y - screen_frame_offset + 1, cabinet_front_left_x + 1, '=======================')

        # Left cabinet border (sides of the screen part)
        for y in range(self.start_y - screen_frame_offset - 1, self.start_y + self.game_height + screen_frame_offset + 1):
            self.add_char_safe(y, cabinet_front_left_x - 1, '*')
        
        # Right "depth" line of the cabinet (continuous vertical line)
        cabinet_total_height = (self.start_y - screen_frame_offset) + (self.game_height + 1) + (screen_frame_offset + 1) + 4 + 8 + 1 # Calculate total height based on elements
        for y in range(self.start_y - screen_frame_offset + 1, cabinet_total_height - 1): # Start 1 char down to allow for top slant
        ...
        for i in range(cabinet_depth - 1): # Connect across the depth
            self.add_string_safe(self.start_y + self.game_height + screen_frame_offset - 1 + i, 
                               cabinet_front_right_x + i - 1, '\\')

        # Top perspective line of the cabinet from top-right front to top-right depth
        self.add_char_safe(self.start_y - screen_frame_offset, right_depth_x, '\\') # Top right corner of the cabinet
        ...
        control_panel_top_y = self.start_y + self.game_height + screen_frame_offset + 1
        ...
        for enemy in self.enemies:
            if enemy['alive']:
                ...
                if enemy['y'] > self.game_height:
                    self.running = False
                    break
                self.add_char_safe(self.start_y + enemy['y'], self.start_x + enemy['x'], '👾')
    ...
    def check_enemies(self):
        ...
        if not any(enemy['alive'] for enemy in self.enemies):
            ...
            self.add_string_safe(mid_screen_y, mid_screen_x, '🎉') 
...
def main():
    ...
    try:
        ...
        # Width: left cabinet edge + game_width + right screen frame column + depth line
        required_width = (game.start_x - screen_frame_offset) + (game.game_width + 1) + (screen_frame_offset + cabinet_depth) + 1 
        
        # Height: top cabinet edge + game_height + bottom screen slant + control_panel + body + base + small buffer
        required_height = (game.start_y - screen_frame_offset) + (game.game_height + 1) + (screen_frame_offset + 1) + 4 + 8 + 1 + 2
